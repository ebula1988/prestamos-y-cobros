  ...
  $.ajax({
     ...
     .done(function(resp) {
      ...
      $("#montoCobrado").html("Monto Cobrado "+monto+"$");
           var aux = montoPrestado + (montoPrestado * .20);
           total = aux - montoCobrado;
...
function ordenar(){
	...
	var index;
	var flag = false;
	$('#tbllistado tbody').on( 'click', 'img', function () {
		if(flag) return;
		if(is_ordening_tbl){
		...
		var posicion = prompt("Por favor ingrese la posición al que desea subir", "0");
		if (posicion != null) {
